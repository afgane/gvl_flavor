"""
A script to automate installation to tool repositories from a Galaxy Tool Shed
into an instance of Galaxy.
Installed tools need to be provided in YAML format in a file called ``tool_list.yaml``.
See ``tool_shed_tool_list.yaml.sample`` for a sample of such a file.
...
for r in r_info:
    ...
    print 'Installing tool %s from %s' % (r['name'], r['owner'])
    ...
    if 'error' in response:
        print "Tool install error! Name: %s, owner: %s, error: %s" % (r['name'],
            r['owner'], response['error'])
    else:
        print "Tool %s installed successfully (in %s)" % (r['name'], str(end - start))
